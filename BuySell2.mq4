...
extern int                 BeginNewSell=0;//Bat dau lai sell tu dau (0 la chay tiep history, 1 la chay tu dau 0.01 lot)
extern int                 BeginNewBuy=0;//Bat dau lai buy tu dau (0 la chay tiep history, 1 la chay tu dau 0.01 lot)
                     ...
                     {  
                                 if(BeginNewBuy==0)
                                 {
                                          for(count=1;count<OrdersHistoryTotal();count++)
                                          {
                                                   ...
                                                   if((OrderSymbol()==Symbol())&&(OrderType()==OP_BUY)) break; //Find last order with the same currency
            ...
            if(SellOrders()==0)//If no order open
               {
                     ...
                     if(OrdersHistoryTotal()!=1)
                     {
                           if(BeginNewSell==0)
                           {
                                    ...
                                    {
                                             OrderSelect(OrdersHistoryTotal()-count,SELECT_BY_POS,MODE_HISTORY);
                                             if((OrderSymbol()==Symbol())&&(OrderType()==OP_SELL)) break; //Find last order with the same currency
...
int BuyOrders()
{
   ...
   { 
      ...
      { 
            if((OrderSymbol()==Symbol())&& (OrderType()==OP_BUY)) buyorders++;
      } 
   }  
   return(buyorders); 
}
...
int SellOrders()
{
   ...
   { 
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) 
      {
            if((OrderSymbol()==Symbol())&&(OrderType()==OP_SELL)) sellorders++;
      } 
   }  
   return(sellorders); 
}